...
public:
    ...
    /* Modified to take the array and num as argument so the strip can be chosen!
    But got little bit laggier but works for any now !*/
    virtual void DrawFire(CRGB* leds, int numLeds)
    {
        ...
        for (int i = 0; i < Size; i++)
        {
            ...
            DrawPixels(j, 1, color, leds, numLeds);
            if (bMirrored)
            {
                int j2 = !bReversed ? (2 * Size - 1 - i) : Size + i;
                DrawPixels(j2, 1, color, leds,  numLeds);
            }
        }
    }
};
